      ...
      <div class="teams_list">
        <v-card class="team_item" v-for="team in existing_teams" :key="team.id">
          <div class="team_name">
            <v-card-title
              class="team_name_"
              v-on:click="redirectTeamInfo(team.id)"
            >
              {{ team.name }}
            </v-card-title>
            <v-card-subtitle>
              {{ count_members(team.id) }} members
            </v-card-subtitle>
            <v-btn
              class="vdelete_btn"
              elevation="3"
...
export default {
  ...
  methods: {
    async list_teams() {
      ...
      axios
        .get("http://localhost:4000/api/teams", config)
        .then((response) => {
    ...
    async createTeam() {
      ...
      axios
        .post("http://localhost:4000/api/teams", data, config)
        .then((response) => {
    ...
    count_members(teamID) {
      ...
      axios
        .get("http://localhost:4000/api/userTeams/team/" + teamID, config)
        .then((res) => {
          this.countTeam = Object.keys(res?.data?.users)?.length;
    ...
    deleteTeam(teamID) {
      ...
      axios
        .delete("http://localhost:4000/api/teams/" + teamID, config)
        .then((res) => {
          console.log(res);
          this.$notify({
            title: "Success",
